...
if ($RequestLiveHomeAssistant) {
  ...
  if ($ticket.complete) {
    ...
    Add-CommandLayer `
      -Id "FIV-12b" `
      -Name "Home Assistant allowed action state check" `
      -ProofLayer "home-assistant/state-check" `
      -ScriptName "start-home-control-bridge.ps1" `
      -Arguments @("-CheckState", "-ActionId", $AllowedActionId) `
      -PassDetail "allowed action state check completed; no physical action executed" `
      -FailureDetail "allowed action state check failed" | Out-Null

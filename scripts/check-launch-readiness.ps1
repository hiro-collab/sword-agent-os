...
$controlPlaneRoot = Resolve-WorkspacePath "control-plane/core"
...
$checks += Test-PathCheck -Id "local.mediapipe_gesture_model" -Path (Join-Path $mediapipeRoot "gesture_model.pkl") -MissingSeverity "warning" -MissingDetail "camera_gesture_lane=held: gesture_model.pkl is local-only and required for Camera Hub gesture classification, but it is not required for the device-free owner conversation/avatar route. If you already have the model, place it at organs/reflex/mediapipe-sword-sign/gesture_model.pkl. Without it Camera Hub can fail with model_not_found and Camera Hub topics timeout; keep Camera/Gesture proof held."

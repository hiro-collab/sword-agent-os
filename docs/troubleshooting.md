...
| Environment State / `VISION LIGHT` が review で信頼できるか不安 | `scripts/check-rr003-env-state-review-preflight.ps1` を使い、`/environment/current` の周期更新、source freshness、room-light evidence の変化、HUD の `Action Mode` を分けて確認します。`unknown/daylight` や low-confidence のままなら `available-but-not-reliable-for-electric-on-off` と扱い、電灯 ON/OFF proof とは主張しません。`Action Mode UNKNOWN` は runtime/status wiring または再起動反映の gap です。`Bridge OK` は接続状態だけで、live Home Control mode の証明ではありません。 |

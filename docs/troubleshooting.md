...
| カメラが1フレーム読めたのに家電 proof にならない | no-save camera check は観測経路の readiness だけを示します。家電の物理状態 proof にするには、保存しない redacted summary の範囲、対象 action、観測タイミング、stop 条件を別途決めた scoped observation run が必要です。toggle-only light は true on/off proof として扱わず、現行の Home Control config では `light_toggle` を open-loop / external-observation-required の action として扱います。standard stack が MediaPipe/VSP でカメラを掴んでいる時は、独立 no-save observation route を明示します。 |

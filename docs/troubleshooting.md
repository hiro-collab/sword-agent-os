...
| Environment State / `LIGHT EST` が review で信頼できるか不安 | `scripts/check-rr003-env-state-review-preflight.ps1` を使い、`/environment/current` の周期更新、source freshness、`vision.room_light` の `observation_bucket` (`dark|dim|balanced|bright`) と cue likelihood (`warm_light` / `daylight` / `darkness`) の変化、`daylight_ambiguity`、HUD の `Action Mode` を分けて確認します。非canonicalまたは stale な観測は `unavailable` です。canonical かつ non-stale でも numeric `confidence` が `0.7` 未満、または `daylight_ambiguity` が `low` 以外なら `available-but-not-decisive-camera-environment-estimate` と扱います。有効な bucket、`confidence >= 0.7`、`daylight_ambiguity=low` が揃って初めて high-confidence claim の候補です。この surface は camera/environment estimate に限られ、electric light state truth、physical switch position truth、physical room illuminance を証明しません。`Action Mode UNKNOWN` は runtime/status wiring または再起動反映の gap です。`Bridge OK` は接続状態だけで、live Home Control mode の証明ではありません。 |
...
| カメラが1フレーム読めたのに家電 proof にならない | no-save camera check は観測経路の readiness だけを示します。家電の物理状態 proof にするには、保存しない redacted summary の範囲、対象 action、観測タイミング、stop 条件を別途決めた scoped observation run が必要です。現行の demo-safe light command stimulus は `light_on` の command submission だけを扱い、true on/off proof や HA state proof には格上げしません。standard stack が MediaPipe/VSP でカメラを掴んでいる時は、独立 no-save observation route を明示します。 |

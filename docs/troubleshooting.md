...
| カメラが1フレーム読めたのに家電 proof にならない | no-save camera check は観測経路の readiness だけを示します。家電の物理状態 proof にするには、保存しない redacted summary の範囲、対象 action、観測タイミング、stop 条件を別途決めた scoped observation run が必要です。 |

...
calls, browser/camera operations, or live-authority bypasses.
...
| `REQUIRE_APPROVAL` | Future marker for routes that need explicit human confirmation before mutation. | Reserved vocabulary |
...
- The marker is not a live-operation token and does not bypass a later live ticket.
...
provider calls, browser/camera operations, or live-authority bypasses by itself.
